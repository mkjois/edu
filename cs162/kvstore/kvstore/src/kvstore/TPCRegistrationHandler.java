...
public class TPCRegistrationHandler implements NetworkHandler {
	...
	try {
	    RegistrationHandler runnableHandler = new RegistrationHandler(slave);
    ...
    public class RegistrationHandler implements Runnable {
        ...
        public void run() {
            KVMessage response;
            try {
        	KVMessage slaveRequest = new KVMessage(slave);
        	TPCSlaveInfo slaveInfo = new TPCSlaveInfo(slaveRequest.getMessage());
        	master.registerSlave(slaveInfo);
        	response = new KVMessage(RESP, "Successfully registered " +
