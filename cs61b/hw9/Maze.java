...
public class Maze {
  ...
  public Maze(int horizontalSize, int verticalSize) {
    ...
    DisjointSets cells = new DisjointSets(this.horiz * this.vert);
    int[] walls = this.makeWallArray(), pos;
    int cell1, cell2, root1, root2;
    boolean connects = false;
    for (int w : walls) {
        pos = this.unHashWall(w);
        cell1 = this.hashCell(pos[0], pos[1]);
        root1 = cells.find(cell1);
        if (pos[2] == HWALL) {
            cell2 = this.hashCell(pos[0], pos[1]+1);
        } else {
            cell2 = this.hashCell(pos[0]+1, pos[1]);
        }
        root2 = cells.find(cell2);
        if (root1 != root2) {
            if (pos[2] == HWALL) {
                this.hWalls[pos[0]][pos[1]] = false;
            } else {
                this.vWalls[pos[0]][pos[1]] = false;
            }
            connects = true;
        } else {
            connects = false;
  ...
  // returns the number of the cell, starting from 0 at the top-left,
  // moving left to right, then top-down
  private int hashCell (int h, int v) {
      return this.horiz * v + h;
  }
  
  private static final int HWALL = 0;
  private static final int VWALL = 1;

  // returns an int[3] whose elements are horizontal position,
  // vertical position, and whether the wall is horizontal
  // or vertical, defined by final constants
  private int[] unHashWall (int n) {
      int[] result = new int[3];
      if (n < this.numHWalls()) { // n is a horizontal wall
          result[0] = posMod(n, this.horiz);
          result[1] = n / this.horiz;
          result[2] = HWALL;
      } else { // n is a vertical wall
          n -= this.numHWalls();
          result[0] = posMod(n, this.horiz-1);
          result[1] = n / (this.horiz-1);
          result[2] = VWALL;
      }
      return result;
  }

  // returns the number of horizontal walls in this maze
  private int numHWalls () {
      return horiz * (vert-1);
  }
  
  // returns the number of vertical walls in this maze
  private int numVWalls () {
      return vert * (horiz-1);
  }

  private int[] makeWallArray () {
      int[] walls = new int[this.numHWalls() + this.numVWalls()];
      for (int i = 0; i < walls.length; i++) {
          walls[i] = i;
      }
      int w = walls.length, temp, rand;
      while (w > 1) { // randomization
          rand = randInt(w);
          temp = walls[w-1];
          walls[w-1] = walls[rand];
          walls[rand] = temp;
          w--;
      }
      return walls;
