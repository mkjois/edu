...
def enhancedFeatureExtractorDigit(datum):
    ...
    features['oneWhitespace'] is 1 if there is only one continuous white area, 0 if not
    ##
    """
    features =  basicFeatureExtractorDigit(datum)

    "*** YOUR CODE HERE ***"
    features["oneWhitespace"] = 1 if oneWhitespace(datum) else 0

    return features

def oneWhitespace(datum):
  ...
  while not len(frontier) == 0:
    ...
    for n in neighbors:
      if n[0] < 0 or n[0] >= DIGIT_DATUM_WIDTH: continue
      if n[1] < 0 or n[1] >= DIGIT_DATUM_HEIGHT: continue
      if datum.getPixel(n[0], n[1]) > 0: continue
      if n in explored or n in frontier: continue
      frontier.insert(0, n)
  return whitePixels == countWhitePixels(datum)

def countBlackPixels(datum):
  count = 0
  for x in range(DIGIT_DATUM_WIDTH):
    for y in range(DIGIT_DATUM_HEIGHT):
      if datum.getPixel(x, y) > 0:
...
def countWhitePixels(datum):
  return DIGIT_DATUM_WIDTH * DIGIT_DATUM_HEIGHT - countBlackPixels(datum)
...
def enhancedPacmanFeatures(state, action):
    ...
    "*** YOUR CODE HERE ***"
    successor = state.generateSuccessor(0, action)
    pacpos1 = state.getPacmanPosition()
    pacpos2 = successor.getPacmanPosition()
    food = successor.getFood()
    walls = successor.getWalls()

    closestGhostBefore = food.width + food.height
    closestGhostAfter = closestGhostBefore
    for x, y in state.getGhostPositions():
      dist = util.manhattanDistance(pacpos1, (x,y))
      if dist < closestGhostBefore:
        closestGhostBefore = dist
    for x, y in successor.getGhostPositions():
      dist = util.manhattanDistance(pacpos2, (x,y))
      if dist < closestGhostAfter:
        closestGhostAfter = dist
    features["closerGhost"] = 1 if closestGhostAfter < closestGhostBefore else 0

    closestFoodBefore = food.width + food.height
    closestFoodAfter = closestFoodBefore
    for x, y in state.getFood().asList():
      dist = util.manhattanDistance(pacpos1, (x,y))
      if dist < closestFoodBefore:
        closestFoodBefore = dist
    for x, y in successor.getFood().asList():
      dist = util.manhattanDistance(pacpos2, (x,y))
      if dist < closestFoodAfter:
        closestFoodAfter = dist
    features["closerFood"] = 1 if closestFoodAfter < closestFoodBefore else 0
