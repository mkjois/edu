...
public class PriorityScheduler extends Scheduler {
    ...
    public static void selfTestMJ() {
        ...
        int integrationTestIterations = 10;
    ...
    private static boolean integrationTest() {
        ...
        for (int i=0; i < numThreads; i++) {
            if (random.nextBoolean()) {
                ...
                // priorites by index mod 8 (i.e. thread1 is 1, thread2 is 2, etc.)
        ...
        for (int i=0; i < numThreads; i++) {
            if (i < numSpeakers) {
                rarr[i] = new SpeakRunnable(comm, alarm);
            } else {
                rarr[i] = new ListenRunnable(comm, alarm);
            }
        ...
        return spokenWords.equals(receivedWords);
    }
...
class SpeakRunnable implements Runnable {
    ...
    public SpeakRunnable(Communicator comm, Alarm alarm) {
        this.comm = comm;
        this.alarm = alarm;
...
class ListenRunnable implements Runnable {
    ...
    Alarm alarm;
    int heard;
    public ListenRunnable(Communicator comm, Alarm alarm) {
        this.comm = comm;
        this.alarm = alarm;
        this.heard = -1;
    }
