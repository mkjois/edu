...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
        # starting position plus a boolean for each corner visited (same order as self.corners)
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                try:
                    cornerBitIndex = self.corners.index(nextPosition) + 1
                    nextState[cornerBitIndex] = True
                except ValueError:
                    pass
                nextState = tuple(nextState)
                cost = 1
                #cost = dy if action == Directions.NORTH or action == Directions.SOUTH else dx
                successors.append((nextState, action, cost))
...
def cornersHeuristic(state, problem):
    ...
    """
    Description: finds the closest unvisited corner to the current position, then
    finds the next closest unvisited corner to the that corner, then the next
    closest unvisited corner to that corner, etc. Measured by Manhattan distance.
    """
    "*** YOUR CODE HERE ***"
    position = state[0]
    minPath = [position]
    unvisited = [corners[i] for i in range(len(corners)) if not state[i+1]]
    if len(unvisited) == 0:
        return 0
    while len(unvisited) != 0:
        nextCorner = min(unvisited, key=lambda c: util.manhattanDistance(position, c))
        minPath.append(nextCorner)
        unvisited.remove(nextCorner)
        position = nextCorner
    return manhattanPathDistance(minPath)

def manhattanPathDistance(path):
    distances = []
    for i in range(len(path)-1):
        d = util.manhattanDistance(path[i], path[i+1])
        distances.append(d)
