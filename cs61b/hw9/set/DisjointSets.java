...
public class DisjointSets {
  ...
  public void union(int root1, int root2) {
    ...
    if (array[root1] >= 0) {
        invalid = true;
        msg += root1 + " is not a root of its set.\n";
    }
    if (array[root2] >= 0) {
        invalid = true;
        msg += root2 + " is not a root of its set.\n";
    }
    if (root1 == root2) {
        invalid = true;
        msg += "Cannot merge equal roots " + root1 + " and " + root2 + "\n";
    }
    if (invalid) {
  ...
  public int find(int x) {
    if (x < 0 || x >= array.length) {
        System.out.println(x + " is not in this set collection.");
