        ...
        for oldPos, oldProb in self.beliefs.items():
          if oldProb > 0:
            ...
            for newPos, newProb in newPosDist.items():
              newBeliefs[newPos] += newProb * oldProb
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        self.particles = [(pos, self.numParticles // numPositions) for pos in self.legalPositions]
        for i in range(self.numParticles % numPositions):
    ...
    def observe(self, observation, gameState):
        ...
        "*** YOUR CODE HERE ***"
        if noisyDistance is not None:
          reweighted = util.Counter()
          for pos, numParticles in self.particles:
            trueDistance = util.manhattanDistance(pacmanPosition, pos)
            reweighted[pos] = numParticles * emissionModel[trueDistance]
          if reweighted.totalCount() == 0:
            self.initializeUniformly(gameState)
          else:
            reweighted.normalize()
            samplePositions = util.nSample(reweighted.values(), reweighted.keys(), self.numParticles)
            intermediate = util.Counter()
            for pos in samplePositions:
              intermediate[pos] += 1
            self.particles = intermediate.items()
        else:
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos, numParticles in self.particles:
          newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
          samplePositions = util.nSample(newPosDist.values(), newPosDist.keys(), numParticles)
          for pos in samplePositions:
            newParticles[pos] += 1
        self.particles = newParticles.items()
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        allStates = itertools.cycle(allStates)
        self.particles = []
        for i in range(self.numParticles):
          self.particles.append(allStates.next())
    ...
    def observeState(self, gameState):
        ...
        for particle in self.particles:
          ...
          newFactor = 10000 # this number is large so we don't lose precision
          ...
          for i in range(self.numGhosts):
            if noisyDistances[i] is not None:
              ...
              newFactor *= emissionModels[i][trueDistance]
            elif particle[i] != self.getJailPosition(i):
              newParticle = self.getParticleWithGhostInJail(particle, i)
          reweighted[newParticle] += newFactor

        if reweighted.totalCount() == 0:
          self.initializeParticles()
        else:
          reweighted.normalize()
          self.particles = util.nSample(reweighted.values(), reweighted.keys(), self.numParticles)
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
              cacheKey = (oldParticle, i)
              newPosDist = None
              if cacheKey in self.distributionCache:
                newPosDist = self.distributionCache[cacheKey]
              else:
                newPosDist = getPositionDistributionForGhost(
                  setGhostPositions(gameState, oldParticle), i, self.ghostAgents[i]
                )
                self.distributionCache[cacheKey] = newPosDist
