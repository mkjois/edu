...
public class TPCLog {
	...
	for (KVMessage msg : entries) {
	    ...
	    if (msg.getMsgType().equals(DEL_REQ)) {
		if (kvServer.hasKey(msg.getKey())) {
	...
	/*
	//Last msg was a commit so need to send response
	if (entries.get(entries.size()-1).getMsgType().equals(COMMIT)) {
	    KVMessage response = new KVMessage(ACK);
	    entries.add(response);
	}
	*/
	
	//Not sure if flushing needed
	//mj: I don't think it is, since the log isn't modified
	//flushToDisk();
    }
