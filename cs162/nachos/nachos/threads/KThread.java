...
public class KThread {
    ...
    public static void finish() {
        ...
        if (currentThread.joined != null) {
            KThread nextThread = currentThread.joined.nextThread();
            nextThread.ready();
        }
    ...
    public void join() {
        ...
        if (status != statusFinished) {
            joined = ThreadedKernel.scheduler.newThreadQueue(true);
    ...
    public static void selfTest() {
        ...
        System.out.print("\nInitiating kernel thread join test...");
        ...
        for (int i = 0; i < 10; i++) {
            ...
            tester.j = i;
            tester.testLock = testLock;
            tester.list = list;
            arr[i] = new KThread(tester);
            if (i > 0) {
                tester.toJoin = arr[i-1];
            }
        }
        for (int i = 0; i < 10; i++) {
            // Jumble up the forking order.
            arr[(i*7)%10].fork();
        }
        arr[9].join();
        int counter = 0;
...
class JoinTester implements Runnable {
    public int j;
    public Lock testLock;
    public KThread toJoin;
    ...
    public void run() {
        ...
        this.testLock.acquire();
        this.list.add(new Integer(j));
