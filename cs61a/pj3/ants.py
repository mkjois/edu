        ...
        if not insect.is_ant():
            ...
            if hasattr(insect, 'true_queen'):
                if insect.true_queen:
                    return '{0} cannot be removed'.format(insect)
            self.ant = None
            if hasattr(insect, 'ant'):
                self.ant = insect.ant
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        if self.place.ant is None: return False
...
class Ant(Insect):
    ...
    def can_contain(self, other):
        is_container = self.container
        if not is_container: return False
        doesnt_contain_another = self.ant is None
        if not doesnt_contain_another: return False
        if other is None: return True
        other_isnt_container = not other.container
        return is_container and doesnt_contain_another and other_isnt_container
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        """
        current_place, i, j = self.place, self.min_range, self.min_range
        while i > 0:
            if not isinstance(current_place, Hive):
                current_place = current_place.entrance
            i -= 1
        target = current_place.bees
        while ((current_place.name != 'Hive') and (not target)
            and (j <= self.max_range)):
                target = current_place.bees
                current_place = current_place.entrance
                j += 1
        return random_or_none(target)
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, colony):
        ...
        if not self.true_queen:
            ...
            prev_queen = colony.queen
            colony.queen = QueenPlace(prev_queen, self.place)
            x2_damage_ants, check_place = [], self.place.exit
            while check_place is not None:
                if check_place.ant is not None:
                    x2_damage_ants.append(check_place.ant)
                check_place = check_place.exit
            for ant in x2_damage_ants:
                if not ant.x2_damage:
                    ant.damage *= 2
                    ant.x2_damage = True
            ThrowerAnt.action(self, colony)

class QueenPlace(object):
    """The Place of the QueenAnt."""

    def __init__(self, prev_queen, curr_place):
        self.prev_queen = prev_queen
        self.curr_place = curr_place

    @property
    def bees(self):
        return self.prev_queen.bees + self.curr_place.bees
