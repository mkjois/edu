    ...
    public static class Map1 extends Mapper<WritableComparable, Text, Text, DoublePair> {
        ...
        @Override
            ...
            throws IOException, InterruptedException {
                ...
                while (matcher.find()) {
                    counter += 1.0;
                    String currWord = matcher.group().toLowerCase();
                    if (currWord.equals(this.targetGram)) {
                        while (words.size() > 0 && distances.size() > 0) {
                            Text word = new Text(words.poll());
                            double dist = distances.poll().doubleValue();
                            if (! hasTargetWord) { // first encounter of target word
                                dist = counter - dist;
                            } else {
                                dist = Math.min(dist, counter - dist);
                            }
                            context.write(word, new DoublePair(1.0, func.f(dist)));
                        }
                        hasTargetWord = true;
                        counter = 0.0;
                    } else {
                ...
                // finish up words after last occurrence of target word
                while (words.size() > 0 && distances.size() > 0) {
                    Text word = new Text(words.poll());
                    double dist = distances.poll().doubleValue();
                    if (! hasTargetWord) {
    ...
    public static class Combine1 extends Reducer<Text, DoublePair, Text, DoublePair> {
        ...
        @Override
            public void reduce(Text key, Iterable<DoublePair> values,
                ...
                double aValue = 0.0, sValue = 0.0; // a_w and s_w
                for (DoublePair pair : values) {
                    aValue += pair.getDouble1();
                    sValue += pair.getDouble2();
                }
                context.write(key, new DoublePair(aValue, sValue));
    ...
    public static class Reduce1 extends Reducer<Text, DoublePair, DoubleWritable, Text> {
        @Override
            public void reduce(Text key, Iterable<DoublePair> values,
                ...
                double result;
                if (sValue > 0.0) {
                    result = (sValue / aValue) * Math.pow(Math.log(sValue), 3.0);
                } else {
                    result = 0.0;
                }
                context.write(new DoubleWritable(-1.0 * result), key);
        ...
        @Override
            public void reduce(DoubleWritable key, Iterable<Text> values,
                ...
                for (Text word : values) {
                    ...
                    context.write(new DoubleWritable(key.get() * -1.0), word);
                    n++;
                }
